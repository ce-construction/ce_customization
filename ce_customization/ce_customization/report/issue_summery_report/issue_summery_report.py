...
class IssueSummary(object):
	...
	def get_common_filters(self):
		...
		for entry in ["company", "status", "priority", "user_name", "project"]:
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						...
						self.issue_summary_data.setdefault(entry, frappe._dict()).esetdefault(agreement_status, 0.0)
...
# def execute(filters=None):
#     columns = [
#         {'fieldname': 'user_name', 'label': 'User', 'fieldtype': 'Data'},
#         {'fieldname': 'first_response_time', 'label': 'First Response Time', 'fieldtype': 'Date'}
#     ]
#     data = frappe.db.get_all('Issue', ['user_name','first_response_time'])
